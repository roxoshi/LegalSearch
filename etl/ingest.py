...
from etl.database import init_db, Document
...
def main():
    ...
    parser.add_argument("--json-dir", required=True, help="Directory containing JSON metadata files")
    parser.add_argument("--pdf-dir", required=True, help="Directory containing Source PDF files")
    parser.add_argument("--limit", type=int, default=0, help="Limit number of documents to process (0 for all)")
    ...
    with SessionLocal() as db:
        for json_file in json_files:
            try:
                ...
                db.commit()
                success_count += 1
                logger.info(f"Successfully ingested {doc_json.case_id} with {len(db_chunks)} chunks.")
